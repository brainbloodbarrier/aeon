...
function getPool() {
  if (!pool) {
    if (!process.env.DATABASE_URL) {
      throw new Error('[CounterforceTracker] DATABASE_URL environment variable is required');
    }
    ...
    pool.on('error', (err) => {
      console.error('[CounterforceTracker] Unexpected error on idle client', err);
    });
  }
