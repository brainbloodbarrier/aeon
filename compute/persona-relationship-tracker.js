...
function getPool() {
  if (!pool) {
    if (!process.env.DATABASE_URL) {
      throw new Error('[PersonaRelationshipTracker] DATABASE_URL environment variable is required');
    }
    ...
    pool.on('error', (err) => {
      console.error('[PersonaRelationshipTracker] Unexpected error on idle client', err);
    });
  }
