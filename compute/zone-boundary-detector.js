...
function getPool() {
  if (!pool) {
    if (!process.env.DATABASE_URL) {
      throw new Error('[ZoneBoundaryDetector] DATABASE_URL environment variable is required');
    }
    ...
    pool.on('error', (err) => {
      console.error('[ZoneBoundaryDetector] Unexpected error on idle client', err);
    });
  }
